...
export const MusicBar = observer(({ music, config }: Props) => {
  ...
  const { isShuffling, playingMode } = config
  return <Paper square sx={{ px: 2 }}>
    <Stack direction="row" alignItems="center" justifyContent="space-between">
      ...
      <Stack sx={{ flex: 0.5 }} alignItems="center">
        <Stack direction="row" alignItems="center" spacing={1}>
          ...
          <Stack sx={{ position: 'relative' }}
              color={playingMode % 3 === 0 ? 'error.main' : 'text.primary'} alignItems="center" onClick={() => config.next('playingMode')}
          >
      ...
      <Stack direction="row" spacing={1}>
        ...
        <Stack direction="row">
          <VolumeDownIcon />
          <Slider size="small" defaultValue={50} min={0} max={100} sx={{ ...sliderSx, width: '120px' }}/>
        </Stack>
        <OpenInFullIcon />
      </Stack>
    </Stack>
